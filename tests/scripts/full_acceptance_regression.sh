...
WORKDIR="${WORKDIR:-/tmp/nbhx_full_acceptance_$RUN_ID}"
...
PY
  then
    warn "当前 Python 环境（$PYBIN）未安装 websockets，跳过 WebSocket 测试"
    return
  fi
...
PY
  ...
  MODE=ok "$PYBIN" "$WORKDIR/ws_reject.py" > "$WORKDIR/ws_ok.out" 2>&1 || true
